...
Filmes.getListaAtores = async function(){
    var query = `select ?idAtor ?nome ?sexo where {
        ?a a c:Ator.
    	?a c:nome ?nome.
    	bind(strafter(str(?a), 'cinema#') as ?idAtor) .
    	?a c:sexo ?sexo.	
...
Filmes.getFilmesDoAtor = async function(idAtor){
    var query = `select ?f where {
        c:${idAtor} a c:Ator.
    	c:${idAtor} c:atuou ?flink.
    	?flink c:título ?f.
    }` 
    var encoded = encodeURIComponent(prefixes + query)
...
Filmes.getPersonagensDoAtor = async function(idAtor){
    var query = `select ?p where {
        c:${idAtor} a c:Ator.
    	c:${idAtor} c:representa ?plink.
    	?plink c:nome ?p.
    }` 
    var encoded = encodeURIComponent(prefixes + query)
...
Filmes.getAtor = async function(idAtor){
    try{
        ...
        var persona = await Filmes.getPersonagensDoAtor(idAtor)
